...
export function ReservationHistoryTable({ reservations, isLoading, stats, onRefresh, onCancel, isCancelling, labels }: ReservationHistoryTableProps) {
  ...
  const rows = useMemo(() => reservations
    ...
    .sort((a, b) => {
      // The API already returns this order, but keep it here too because the
      // filters and the other sort modes operate on the client.
  ...
  const cancelledByLabel = (reservation: Reservation) => {
    if (!reservation.cancelledBy) return labels.unknown;
  ...
  return (
    <section className="space-y-5" aria-label={labels.title}>
      <div className="rounded-2xl border border-primary/25 bg-card/95 p-5 shadow-layered backdrop-blur-sm"><h2 className="text-xl font-semibold text-foreground">{labels.title}</h2><p className="mt-1 text-sm text-muted-foreground">{labels.subtitle}</p></div>
      ...
      <div className="rounded-2xl border border-primary/25 bg-card/95 p-4 shadow-layered backdrop-blur-sm"><div className="flex flex-col gap-3 md:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="pl-10" placeholder={labels.search} /></div><select value={status} onChange={(event) => setStatus(event.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="ALL">{labels.all}</option><option value="PENDING">{labels.pending}</option><option value="CONFIRMED">{labels.confirmed}</option><option value="ARRIVED">{labels.arrived}</option><option value="CANCELLED">{labels.cancelled}</option></select><select value={sort} onChange={(event) => setSort(event.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="date-desc">{labels.newest}</option><option value="date-asc">{labels.oldest}</option><option value="status">{labels.byStatus}</option></select><Button size="icon" onClick={handleRefresh} title={labels.refresh} className="shrink-0"><RefreshCw className="h-4 w-4" /></Button></div></div>
      <div className="overflow-hidden rounded-2xl border border-primary/25 bg-card/95 shadow-layered">{isLoading ? <div className="flex justify-center py-12"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : <Table><TableHeader><TableRow><TableHead>{labels.customer}</TableHead><TableHead>{labels.date}</TableHead><TableHead>{labels.guests}</TableHead><TableHead>{labels.status}</TableHead><TableHead className="text-right">{labels.actions}</TableHead></TableRow></TableHeader><TableBody>{rows.length ? rows.map((reservation) => { const appearance = statusAppearance(reservation.status); const StatusIcon = appearance.icon; const reason = cancellationReason(reservation); return <TableRow key={reservation.id} className="border-border/70 hover:bg-accent/60 dark:hover:bg-accent/30"><TableCell><Tooltip><TooltipTrigger asChild><div className="max-w-40 cursor-default"><p className="truncate font-medium">{reservation.name}</p><p className="truncate text-xs text-muted-foreground">{reservation.phone}</p></div></TooltipTrigger><TooltipContent>{reservation.name} · {reservation.phone}</TooltipContent></Tooltip></TableCell><TableCell>{new Date(reservation.reservationDate).toLocaleDateString(labels.locale)}<br /><span className="text-xs text-muted-foreground">{reservation.reservationTime}</span></TableCell><TableCell>{reservation.numberOfGuests}</TableCell><TableCell><span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ${appearance.className}`}><StatusIcon className="h-3.5 w-3.5" />{statusLabel(reservation.status)}</span>{reservation.status === RESERVATION_STATUS.CANCELLED && <div className="mt-2 max-w-56 rounded-md border border-destructive/20 bg-destructive/5 p-2 text-xs text-foreground"><p>{labels.cancelledBy}: {cancelledByLabel(reservation)}</p>{reason && <Tooltip><TooltipTrigger asChild><p className="mt-1 cursor-default truncate"><span className="font-semibold">{labels.reason}:</span> {reason}</p></TooltipTrigger><TooltipContent className="max-w-xs">{reason}</TooltipContent></Tooltip>}</div>}</TableCell><TableCell className="text-right">{reservation.status === RESERVATION_STATUS.PENDING && <Button variant="outline" size="sm" onClick={() => setSelected(reservation)}>{labels.cancel}</Button>}</TableCell></TableRow>; }) : <TableRow><TableCell colSpan={5} className="py-12 text-center text-muted-foreground"><Calendar className="mx-auto mb-3 h-8 w-8" />{labels.empty}</TableCell></TableRow>}</TableBody></Table>}</div>
